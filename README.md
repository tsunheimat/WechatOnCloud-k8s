...
在飞牛 NAS（x86_64 / arm64）、任意 Docker 主机或 Kubernetes 集群上运行服务端微信：面板可管理**多个**实例，每个实例运行在独立容器或 StatefulSet/Pod 中——可以是一个**微信**会话，也可以是一个 **Chromium 浏览器**（用来登录 Telegram / X / Instagram 等网页版应用）。多个 web 用户通过浏览器访问被授权的实例，实现跨设备同步、多端共享。**不修改微信客户端。**

**一句话原理**：每个实例 = 一个隔离运行单元，里面跑 Xvfb 虚拟显示 + 一个应用（官方原版微信，或 Chromium 浏览器），KasmVNC 把画面串到浏览器；同一实例被多个浏览器连 = 共享同一个会话。前面一层自研**面板**是唯一对外入口，经 Docker Engine 或 Kubernetes API 按需创建/销毁实例并反向代理。
...
- 🗂️ **多实例** — 一个面板管理多个独立实例；Docker 模式下每个实例是独立容器 + 独立数据卷，Kubernetes 模式下每个实例是独立 StatefulSet/Pod + PVC，互不干扰。
...
- 📎 **文件传输 + 中文 / 文本输入** — 拖拽上传 + 下载 + 删除，直达实例桌面 `~/Desktop`；文本可经剪贴板或底部输入条中转送进实例（局域网 http 下也可用）。
- 🔊 **声音 / 麦克风 / 摄像头** — 实例工具栏可手动开启声音与麦克风；摄像头可通过宿主 v4l2loopback 设备映射进实例（麦克风 / 摄像头需 HTTPS 或 localhost）。
- 🧩 **多端协作软锁** — 同一实例多人操作时自动只读 + 申请接管，避免键鼠打架。
- 🩺 **日志诊断 + 自愈** — 实例日志持久化、诊断包导出；watchdog 可按 soft / hard 内存阈值自动重启内存异常增长的实例，也可配置 VNC 响应性探测。
- 🔑 **SSO / OIDC 可选** — 本地账号密码保留，可接入 Authentik / Keycloak / Google / Microsoft Entra 等标准 OIDC；首次 SSO 登录可新建账号或绑定已有账号。
- 🔒 **安全优先** — 面板为唯一入口，KasmVNC 凭据服务端注入、永不下发前端；Host 白名单防 DNS rebinding；Docker 模式的 docker.sock 操作和 Kubernetes 模式的命名空间内资源操作均仅管理员可触达。
- 🔄 **版本检测与更新** — 面板显示当前版本并检测 Docker Hub / GHCR 最新发布；Docker 模式下管理员可一键拉新镜像并重建面板。
...
> **改配置（强烈建议至少改密码）**：默认管理员 **admin / wechat**。登录后在「修改密码」里改；或部署前在 `docker-compose.yml` 旁放一个 `.env`（从 [.env.example](.env.example) 下载改名），又或在 NAS 的 Compose 环境变量里填 `WOC_PASSWORD`、`WOC_HTTP_PORT`、`WOC_IMAGE_PREFIX`、`PANEL_ALLOWED_HOSTS`、`OIDC_*` 等（全部可配置项见 [.env.example](.env.example)）。
...
> **反代 / 域名访问**：面板默认放行 localhost 和内网 IP。若通过 Caddy / Nginx / 飞牛反代 / Cloudflare Tunnel 等域名访问，请把对外域名加入 `PANEL_ALLOWED_HOSTS`，改完用 `docker compose up -d` 重建面板容器（只 `docker restart` 不会加载新环境变量）。SSO/OIDC 回调也依赖这个对外地址，生产环境建议显式配置 `OIDC_REDIRECT_URI`。
...
| 文件 / 文本传输 | 被授权用户 | 拖拽上传 / 下载文件；文本经剪贴板或底部输入条中转送入实例 |
| 声音 / 麦克风 | 被授权用户 | 工具栏手动开启声音；麦克风默认关闭，需 HTTPS 或 localhost 才能被浏览器放行 |
| 实例日志 | 管理员 | 查看实例日志，含**持久化历史**（重启原因 + 上一容器日志快照，跨容器重建保留） |
| 安全阀 / 诊断包 | 管理员 | 按实例设置内存 soft / hard 阈值；导出面板与实例诊断包用于排查 |
| 面板版本 / 更新 | 管理员 | 查看当前版本、检测 Docker Hub / GHCR 最新版；Docker 模式可一键更新面板 |
| 单点登录 / SSO | 所有人 / 管理员 | 配置 OIDC 后登录页显示 SSO；管理员可控制首次登录自助注册、账户绑定和登录按钮图标 |
| 修改密码 | 所有人 | 改自己的登录密码 |
...
- 默认**不设硬性 CPU/内存上限**：空闲时省，活跃时按宿主余量突发；需要兜底时可设 `WOC_INSTANCE_MEM_GB` 给每个实例加硬内存上限（Docker 为 cgroup memory limit，Kubernetes 为 Pod memory limit）。
- 面板默认启用 watchdog：`WOC_INSTANCE_MEM_SOFT_MB=1500` 时仅在无人操作时柔和重启，`WOC_INSTANCE_MEM_HARD_MB=2500` 时强制重启，管理页可按实例覆盖。Kubernetes 模式若没有 metrics-server，内存读数为 0，内存阈值自愈不会触发，仍可依赖 Pod limit / kubelet OOM 重启。
- 每实例另预留 **1 GiB `/dev/shm`**（微信 Chromium 内核所需，tmpfs，按需占用）。
...
能登录面板的人就能看你的聊天记录、以你身份发消息（浏览器实例则能用你登录的 Telegram / X / Instagram 等账号）。在 Docker 部署中，**面板会挂载宿主的 `docker.sock`**（创建/销毁实例所需），它等同宿主 root 权限；在 Kubernetes 部署中，面板通过命名空间内 ServiceAccount/Role 管理实例 Pod、Service、PVC 和 StatefulSet。因此：

- **绝不要把面板裸暴露公网**：只在内网访问，或经飞牛远程访问 / VPN / 内网穿透；
- 务必改掉默认密码（默认 admin / wechat）：`cp .env.example .env` 后改 `WOC_PASSWORD`，或登录后在「修改密码」里改；
- 通过反代或域名访问时，把对外域名加入 `PANEL_ALLOWED_HOSTS`；它是 DNS rebinding 防护的一部分，配置后要用 `docker compose up -d` 重建面板容器；
- 接入 SSO/OIDC 时，仍建议保留一个本地管理员作为 break-glass 账号；SSO 管理员可在面板内禁用 / 删除，但 IdP 侧分组仍要同步收回；
- 实例的增删、微信安装/更新等 runtime 操作**仅限管理员**；Docker API / Kubernetes API 绝不暴露给前端；
...
**用你本地（客户端）的输入法打中文，容器内无需安装任何 IME。** 当前默认使用「转发」模式：先点好微信 / 网页里的目标输入框，再在底部输入条打中文，按 Enter 或「发送」把成品文本粘进实例应用。工具栏也可切到「无感」模式，直接在应用输入框里打中文，提交时由面板捕获成品文本并转发。

- 「转发」模式最稳，局域网 http 下也可用；Shift+Enter 换行。
- 「无感」模式依赖同源 iframe 与浏览器输入法事件，适合想直接在远端输入框里打字的场景；切换输入模式会整页重连一次，让 KasmVNC 以干净状态加载。
...
- [x] Host 白名单防 DNS rebinding + OIDC / SSO 登录（含自助注册 / 绑定）
- [x] 面板版本检测 / Docker 模式一键更新 + watchdog 内存自愈 + 诊断包导出
